...
const OLD_SITE = 'https://www.swiftechie.com/popohu_minilite5/';
const NEW_SITE = 'http://localhost:4321/products/popohu-minilite5';

test.describe('比较popohu-minilite5新旧网站内容', () => {
  test('获取旧网站内容', async ({ page }) => {
    ...
    console.log('\n旧网站主要内容段落:');
    mainContent.slice(0, 15).forEach((p, i) => {
  ...
  test('对比内容差异', async ({ page }) => {
    ...
    // 截图对比
    await page.goto(OLD_SITE, { waitUntil: 'networkidle', timeout: 15000 });
    await page.screenshot({ path: 'old-popohu-minilite5.png', fullPage: true });
    console.log('\n旧网站截图已保存: old-popohu-minilite5.png');

    await page.goto(NEW_SITE, { waitUntil: 'networkidle', timeout: 15000 });
    await page.screenshot({ path: 'new-popohu-minilite5.png', fullPage: true });
    console.log('新网站截图已保存: new-popohu-minilite5.png');
  });
});
